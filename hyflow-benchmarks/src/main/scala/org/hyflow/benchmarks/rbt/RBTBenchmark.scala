...
class RBTBenchmark extends Benchmark {
	val name = "rbt"
...
class RBTree[K <% Comparable[K], V](val _id: String) extends HObj with Logging {
	...
	def delete(key: K)(implicit txn: InTxn) {
		...
		// Sub-functions
	    def maximumNode(n: RBNode[K,V]): RBNode[K,V] = {
			if (n == null)
	    ...
	    def deleteCase1(n: RBNode[K,V]) {
		    if (null != n.parent())
		        deleteCase2(n)
		}
	    
	    def deleteCase2(n: RBNode[K,V]) {
