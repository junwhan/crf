...
class BSTBenchmark extends Benchmark {
	val name = "bst"
	...
	def benchInit() {
		...
		for (i <- slice) {
			val bst = new BSTHandler[Int]("bst"+i)
			bst.createTree(-1)
		}
	}
...
object BSTOps {
	def read(ids: Tuple2[String,Int]*) {
		atomic { implicit txn =>
			for (id <- ids) {
				val bst = new BSTHandler[Int](id._1)
				bst.find(id._2)
			}
		}
	}
...
class BSTHandler[T <% Comparable[T]](val _id: String) extends Logging {
	def header = _id + "-head"
