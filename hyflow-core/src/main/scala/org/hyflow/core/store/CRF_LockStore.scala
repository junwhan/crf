...
object CRF_LockStore extends Service with Logging with LockProvider with StoreProvider {
  ...
  case class CheckMsg(id: String, txid: Long) extends Message
	...
	val name = "crf-lock-store"
	val actorProps = Props[CRF_LockStore_Actor]
...
class CRF_LockStore_Actor extends Actor with Logging {
	
	import CRF_LockStore._
  ...
  private val epoch = mutable.Map[String, Request]()
	...
	def receive() = {
			...
			val resp = m.ids.map { id =>
				...
				val res: Either[HObj, Symbol] = if (entryOpt == None) {
					...
					if (entry.obj == null) {
						...
						log.debug("Object store queried for a locked object. | id = %s | sender = %s",
								id, sender)
            if(m.isRead) Left(entry.obj)
						else Right('locked_object)
					} else {
		...
		case m: PutMsg =>
			...
			entry.obj = m.obj
      entry.versionList += m.obj
      if(entry.versionList.length > 10) entry.versionList.remove(0)
    ...
    case m: CheckMsg =>
      var entry = store.getOrElseUpdate(m.id, new ObjectStoreEntry)
      //entry.depth = 0
      //log.debug("Receive a check message here:  %d to sender %s.", entry.depth, sender)
      sender ! StoreProvider.CheckResp(Some(entry.depth))
		case m: ValidateMsg =>
        ...
        if(entry.depth < 10){
          entry.depth += 1
          log.warn("Validation %d in CRF", entry.depth)
          true
        }
				else {
      ...
      if(!granted) {
        // a non-commutative tx is enqueued for the next epoch
        var req = epoch.getOrElseUpdate(m.fid._1, new Request)
        req.tx = m.txnid
      }
      ...
      entry.depth -= 1
			if (m.txnid == entry.lockedBy) {
				...
				log.debug("Lock released for local object by wrong txnid. | fid = %s | lockedBy = %s | txnid = %s",
						m.fid, entry.lockedBy, m.txnid)
			} else {
