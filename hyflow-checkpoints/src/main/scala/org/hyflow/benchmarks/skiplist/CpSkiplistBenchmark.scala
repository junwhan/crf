...
class CpSkiplistBenchmark extends Benchmark {
	val name = "cp-skiplist"
	...
	def benchIter() {
		val ids = randomObjIdSet(CpSkiplistConfig.OPS,
			() => ("skip" + rand.nextInt(totalObjects) -> rand.nextInt(CpSkiplistConfig.KEY_RANGE)))
		if (randomlyReadOnly)
			CpSkiplistOps.read(rand, ids: _*)
		else
			CpSkiplistOps.write(rand, ids: _*)
	}

	def benchCheck() = true
}

object CpSkiplistOps extends Logging {
...
class SkipList[T <% Comparable[T]](val _id: String) extends HObj with Logging {
	...
	private val maxLevel = CpSkiplistConfig.MAX_LEVEL
	...
	private def randomLevel = {
		import scala.math
		val rlevel = (math.log(1.0 - math.random) / math.log(1.0 - CpSkiplistConfig.PROBABILITY)).toInt
		math.min(rlevel, CpSkiplistConfig.MAX_LEVEL)
	}
