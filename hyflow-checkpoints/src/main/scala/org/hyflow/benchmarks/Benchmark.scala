...
abstract class Benchmark extends Logging {
	...
	def runBenchmark() {
		...
		val threads = for (i <- 0 until injectorThreads) yield {
			val t = new InjectorThread(node, i)
			log.debug("Creating thread with loader. | _ = %s", this.getClass.getClassLoader)
	...
	private class InjectorThread(node: Int, id: Int) extends Thread("bench-n%d-t%d".format(node, id)) with Logging {
		log.debug("InjectorThread classLoader. | _ = %s", this.getClass().getClassLoader())
		var opsPerformed = 0
		...
		override def run() {
			log.debug("Inside running thread. | loader = %s | ctx = %s", 
